...
**Found on the live site after the deploy (2026-09-18).** Two faults in the resize path, both present since M1 and both now fixed in `src/main.ts` with a regression test in `e2e/composition.spec.ts`:
- A resize or a phone rotation never reached the renderer. The per-frame size check called the debounced `onResize` on every frame while the window and the applied size disagreed, and each call cleared and re-armed the 80 ms timer, so the callback never ran. The canvas kept its old size and the world was cropped. `onResize` now claims the new size as it schedules, so the check stops firing and the debounce elapses.
- The renderer could start at a size the window never had. The check's baseline was read after `createApp` and `openKv` were awaited, so a viewport that changed during those awaits left the baseline agreeing with the window while the canvas kept the size it was built with. Seen live as a 300x150 scene in a 1024x768 window. The baseline now comes from `renderer.screen`, which is what was actually applied.
