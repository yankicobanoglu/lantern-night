...
import { COTTAGE_A, COTTAGE_B, COTTAGE_PALETTE, COTTAGE_WINDOWS } from './sprites/cottage';
...
export function drawHills(buf: PixelBuffer, layout: Layout, seed: number): Window[] {
  ...
  const cottages = [
    { map: COTTAGE_A, win: COTTAGE_WINDOWS.a, fx: layout.landscape ? 0.2 : 0.22 },
    { map: COTTAGE_B, win: COTTAGE_WINDOWS.b, fx: layout.landscape ? 0.62 : 0.66 },
  ];
  for (const c of cottages) {
    ...
    windows.push({ x: cx + c.win.x, y: cy + c.win.y, w: c.win.w, h: c.win.h });
  }
